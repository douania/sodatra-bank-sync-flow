/*
  # Improve client name extraction and display

  1. New Functions
    - `detect_collection_type`: Function to automatically detect if a collection is a cheque or effet
    - `clean_client_name`: Function to clean client names from descriptions

  2. Triggers
    - Add trigger to automatically detect collection type when no_chq_bd is updated
    - Add trigger to update client names in client_reconciliation
...
-- Add constraint to check excel traceability consistency
...
-- Add unique constraint for excel traceability
...
-- Add column for unique excel traceability
ALTER TABLE collection_report
ADD COLUMN IF NOT EXISTS unique_excel_traceability TEXT GENERATED ALWAYS AS (
  CASE 
