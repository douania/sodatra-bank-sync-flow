...
- Qualification réelle multi-bank : campagne locale du 2026-09-16 sur les fichiers réels de juillet (`GO_VALIDATE_LOCAL_PACK_2_REAL_FILES_JULY_SENSITIVE`) : `FAIL` métier sur les sept familles pour cause de format ; Pack 2 `PACK-2-REAL-FORMAT-COMPATIBILITY` : `IMPLEMENTED_LOCAL — DRAFT_PR` (sélection explicite de feuille, identité par l'en-tête, dates `JJ/MM/AA`, libellés ORA et alias ATB listés, extracteurs tabulaires rapports bancaires et Fund Position, harness `--sheet` / `--real-sensitive-authorized`) ; après contre-revue CTO (`FAIL` sur `a74581b`) et corrections `GO_FIX_PACK_2` (refus de toute ligne après total ou hors section, zone de montant titrée, aucun zéro par défaut, années corroborées, journaux purgés, sélection liée à l'instance), ATB, BICIS et ORA passent localement sur la feuille du 9 juillet, BDK et BIS sont refusés (lignes d'ajustement après le total des facilités, DEF-25), Fund Position refusée (DEF-20, DEF-24), SGBS non testé ; aucune promotion, toutes les familles restent `STAGING_PILOT` ; `GO_VALIDATE_STAGING_PACK_2` refusé ; merge bloqué jusqu'à clôture Pack 0, Pack 0R et nouvelle contre-revue ;
