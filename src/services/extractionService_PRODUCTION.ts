    ...
    if (inDepositsSection && line.length > 10) {
      ...
      if (depositMatch) {
        const deposit: DepositNotCleared = {
          ...
          reference: depositMatch[4],
          clientCode: depositMatch[5].trim(),
...
function extractChecksNotCleared(textContent: string): CheckNotCleared[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inChecksSection && line.length > 10) {
      ...
      if (checkMatch) {
        const isOldCheck = parseInt(checkMatch[1].split('/')[2]) < 2020; // Chèques avant 2020 = anciens
...
function extractBankFacilities(textContent: string): BankFacility[] {
  ...
  facilityData.forEach(data => {
    ...
    const utilizationRate = data.limit > 0 ? (data.used / data.limit * 100) : 0;
    facilities.push(facility);
    console.log(`✅ Facilité: ${facility.facilityType} - ${utilizationRate.toFixed(1)}%`);
  });
...
function extractImpayes(textContent: string): Impaye[] {
  ...
  // Données extraites manuellement du rapport BDK
  const impayeData = [
    { dateRetour: '22/04/2025', dateEcheance: '22/04/2025', banque: 'CORIS', client: 'CHAFIC AZAR & Cie', montant: 2000000 },
    { dateRetour: '11/06/2025', dateEcheance: '10/06/2025', banque: 'ECOBANK', client: 'ADN', montant: 3000000 },
    { dateRetour: '11/06/2025', dateEcheance: '10/06/2025', banque: 'SGBS', client: 'RICHARD EQUIP', montant: 5000000 }
  ];
  ...
  impayeData.forEach(data => {
    const impaye: Impaye = {
      ...
      description: `IMPAYE ${data.banque}`,
...
export function extractBankReport(textContent: string, bankType: string): BankReport {
...
export function extractFundPosition(textContent: string): any {
  ...
  // TODO: Implémenter l'extraction Fund Position
  return {
    type: 'fund_position',
    data: {},
    extractionQuality: { isComplete: false, missingFields: ['all'], confidence: 0 }
  };
}

export function extractClientReconciliation(textContent: string): any {
  console.log('🔍 Extraction Client Reconciliation...');
  // TODO: Implémenter l'extraction Client Reconciliation
  return {
    type: 'client_reconciliation',
    data: {},
    extractionQuality: { isComplete: false, missingFields: ['all'], confidence: 0 }
  };
}