...
const SECTION_DEFINITIONS: SectionDefinition[] = [
  ...
  {
    key: 'totalBalanceA',
    aliases: ['TOTAL BALANCE A', 'TOTAL BALANCE (A)', 'TOTAL A', 'TOTAL (A)', 'SOLDE TOTAL A', 'TOTAL SOLDE A'],
    required: true,
  },
  {
...
class InternalBookExcelParser {
  ...
  private extractAmountLines(
    ...
    let headerRow: NormalizedRow | undefined;
    ...
    for (const row of rows) {
      ...
      const selection = this.selectRightMostMoney(row, sheetName, section, true, headerRow);
      if (selection.issue) {
        issues.push(selection.issue);
      }
  ...
  private extractImpayes(
    ...
    for (const row of rows) {
      ...
      const selection = this.selectRightMostMoney(
        row,
        sheetName,
        'impayes',
        !this.isTotalRow(row),
        this.isTotalRow(row) ? undefined : headerRow,
      );
      if (selection.issue) {
        issues.push(selection.issue);
      }
  ...
  private extractFacilities(
    ...
    for (const row of rows) {
      ...
      const moneyCells = this.getMoneyCandidates(row, 'bankFacilities', headerRow);
  ...
  private selectRightMostMoney(
    ...
    const candidates = this.getMoneyCandidates(row, section, headerRow);
    const money = candidates[candidates.length - 1];

    if (flagAmbiguity && candidates.length > 1 && !this.hasClearRightMostAmount(row, candidates, headerRow, section)) {
      return {
        money,
  ...
  private getMoneyCandidates(
    ...
    const amountHeaderCandidates = moneyCells.filter((money) => this.isAmountColumn(money.columnIndex, headerRow, section));
    if (amountHeaderCandidates.length > 0) {
      return amountHeaderCandidates;
    }

    return moneyCells.filter((money) => !this.isNonAmountColumn(money.columnIndex, headerRow));
  }

  private isNonAmountColumn(columnIndex: number, headerRow: NormalizedRow): boolean {
    const header = this.headerTextForColumn(columnIndex, headerRow);
    return (
      header === 'DATE' ||
  ...
  private isAmountColumn(columnIndex: number, headerRow: NormalizedRow, section?: InternalBookSection): boolean {
    const header = this.headerTextForColumn(columnIndex, headerRow);
    if (/^(AMOUNT|AMOUNT 1|MONTANT|MONTANT 1)$/.test(header)) {
      return true;
    }

    return section === 'bankFacilities' && /^(BALANCE|LIMIT|LIMITE|USED|UTILISE|DISPONIBLE)$/.test(header);
  }

  private headerTextForColumn(columnIndex: number, headerRow: NormalizedRow): string {
    const cell = headerRow.cells.find((candidate) => candidate.columnIndex === columnIndex);
    return cell?.normalizedText ?? '';
  }

  private areHeaderAmountColumns(
    candidates: InternalBookMoneyCell[],
    headerRow: NormalizedRow,
    section?: InternalBookSection,
  ): boolean {
    return candidates.every((candidate) => this.isAmountColumn(candidate.columnIndex, headerRow, section));
  }

  private areColumnsContiguous(candidates: InternalBookMoneyCell[]): boolean {
    const columns = candidates.map((candidate) => candidate.columnIndex).sort((left, right) => left - right);
    return columns.every((column, index) => index === 0 || column === columns[index - 1] + 1);
  }

  private hasClearRightMostAmount(
    row: NormalizedRow,
    candidates: InternalBookMoneyCell[],
    headerRow?: NormalizedRow,
    section?: InternalBookSection,
    ...
    if (headerRow && this.areHeaderAmountColumns(candidates, headerRow, section) && this.areColumnsContiguous(candidates)) {
      return true;
    }
